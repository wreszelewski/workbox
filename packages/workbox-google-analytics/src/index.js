...
/**
 * Defines the requestWillDequeue callback to be used with the background
 * sync queue plugin. The callback takes the failed request and adds the
 * `qt` param based on the current time, as well as applies any other
 * user-defined hit modifications.
 * @param {Object} reqData The request data object retrieved from the
 *     background sync store in IndexedDB. See workbox.backgroundSync.Queue
 *     for more details.
...
export const initialize = (config = {}) => {
  ...
  router.registerRoutes({
    routes: [createAnalyticsJsRoute(), ...createCollectRoutes()],
  });
};
